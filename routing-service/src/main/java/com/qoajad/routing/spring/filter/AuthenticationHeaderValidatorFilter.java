package com.qoajad.routing.spring.filter;

import com.qoajad.routing.service.jwt.JWTService;
...
public class AuthenticationHeaderValidatorFilter extends OncePerRequestFilter {
    ...
    private static final String [] VALIDATE_WHEN_URL_DOESNT_CONTAINS = new String [] {"authentication/"};
    ...
    private String findJWTInHeader(final HttpServletRequest request) {
        String jwtContent = request.getHeader("authentication");
        if (jwtContent == null) {
            return null;
        }
        return jwtContent;
